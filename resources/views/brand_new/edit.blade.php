    ...
    <div class="col-md-8">
      <div class="card">
        <div class="card-header">
          ...
          <div class="card-body">
            <form method="POST" action="{{ url('brand-news/edit/'.$brand_new->id) }}">
              @csrf
              <div class="form-group row">
                <label for="name" class="col-md-12 col-form-label"><h4>{{ __('Titulo') }}</h4></label>
                <div class="col-md-12">
                  <input id="name" type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ $brand_new->name }}" required autofocus>
                  @if ($errors->has('name'))
