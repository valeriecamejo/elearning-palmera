    ...
    <div class="col-md-8">
      <div class="card">
        ...
        <template v-if="countries != ''">
          <div class="card-body">
          <h5 class="card-title">Editar Estado/Provincia {{ $city->name }}</h5>
            <form method="POST" action="{{ url('cities/edit/'.$city->id) }}">
              @csrf
              ...
              <div class="form-group row">
                ...
                <div class="col-md-6">
                <input id="name" type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ $city->name }}" required autofocus>
                @if ($errors->has('name'))
