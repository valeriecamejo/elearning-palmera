	...
	<div class="col-md-8">
        ...
        <ul class="nav nav-tabs card-header-tabs">
          ...
          <div class="form-group row">
            ...
            <img class="card-img-top" src="{{ asset('./storage/marca_' . Auth::user()->id . '/' . $brand->logo) }}" alt="{{ $brand->logo }}">
					</div>
					<div class="form-group row">
						<label for="header" class="col-md-4 col-form-label text-md-right">{{ __('Imagen Header') }}</label>
            <img class="card-img-top" src="{{ asset('./storage/marca_' . Auth::user()->id . '/' . $brand->header) }}" alt="{{ $brand->header }}">
					</div>
			</div>
		</div>
	</div>
